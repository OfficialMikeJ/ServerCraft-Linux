...
class ServerManager:
    ...
    async def start_server(self, server_id: str, game_definitions: Dict, output_callback=None) -> Dict:
        ...
        try:
            ...
            elif game in ("plutonium_t6", "plutonium_iw5"):
                port = server.get("port", game_def.get("default_port", 28961))
                self._generate_plutonium_config(server, server_path, port)
    ...
    def _generate_plutonium_config(self, server: Dict, server_path: Path, port: int) -> None:
        """Write server.cfg and start.sh for a Plutonium (T6/IW5) server."""
        import stat as stat_mod

        game = server.get("game", "plutonium_t6")
        is_t6 = game == "plutonium_t6"

        name        = server.get("name", "ServerCraft Plutonium Server")
        password    = server.get("password", "")
        max_players = server.get("max_players", 18)
        server_key  = server.get("plutonium_key", "")
        game_mode   = server.get("game_mode", "war" if is_t6 else "dm")
        map_name    = server.get("map_name", "mp_nuketown_2020" if is_t6 else "mp_dome")
        extra_cfg   = server.get("extra_cfg", "")

        cfg_lines = [
            f'set sv_hostname "{name}"',
            f'set g_password "{password}"',
            f'set sv_maxclients {max_players}',
            "set g_logSync 2",
            f'set net_port {port}',
            'set sv_pure 1',
            'set sv_cheats 0',
            'set sv_allowClientConsole 1',
            f'set sv_authToken "{server_key}"',
            "",
            "// Map rotation",
            f'set sv_mapRotation "gametype {game_mode} map {map_name}"',
            "",
        ]
        if extra_cfg:
            cfg_lines += ["// Custom config", extra_cfg, ""]

        server_path.mkdir(parents=True, exist_ok=True)
        (server_path / "server.cfg").write_text("\n".join(cfg_lines))

        # Game-specific exe name inside the Plutonium install
        if is_t6:
            win_exe = "t6r.exe"
            game_flag = "+set dedicated 2 +set sv_lanOnly 0"
        else:
            win_exe = "iw5mp_server.exe"
            game_flag = "+set dedicated 2 +set sv_lanOnly 0"

        start_sh = (
            "#!/bin/bash\n"
            "# ServerCraft — Plutonium dedicated server launcher (Wine)\n"
            "cd \"$(dirname \"$0\")\"\n"
            "export WINEDEBUG=-all\n"
            f'wine {win_exe} {game_flag} \\\n'
            f'  +set net_port {port} \\\n'
            f'  +set sv_authToken "{server_key}" \\\n'
            '  +exec server.cfg \\\n'
            '  +map_rotate\n'
        )
        sh_path = server_path / "start.sh"
        sh_path.write_text(start_sh)
        sh_path.chmod(sh_path.stat().st_mode | stat_mod.S_IXUSR | stat_mod.S_IXGRP | stat_mod.S_IXOTH)

        logger.info(f"Generated Plutonium config for server '{name}'")
    ...
    def _build_start_command(self, server: Dict, game_def: Dict, server_path: Path) -> Optional[List[str]]:
        ...
        if game == "arma3":
            ...
            # start.sh is fully generated by _generate_plutonium_config — no extra args needed.
            pass
